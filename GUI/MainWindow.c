...
void MainWindow(GtkWidget *button, gpointer data) {
  ...
  g_signal_connect(button2, "clicked", G_CALLBACK(on_button_clicked_register),
                   window_pass);
  g_signal_connect(window_pass, "delete-event", G_CALLBACK(gtk_main_quit), NULL);
  // 显示窗口和所有控件
...
void on_button_clicked_login(GtkWidget *widget, gpointer data) {
  ...
  gtk_widget_hide(window_parent);
  // 打开登录窗口
  LoginWindow(widget, window_parent);
}
...
void on_button_clicked_register(GtkWidget *widget, gpointer data) {
  // 隐藏父窗口
  GtkWidget *window_parent = (GtkWidget *) data;
  gtk_widget_destroy(window_parent);
  // 打开注册窗口
  RegisterWindow(widget, window_parent);
}
